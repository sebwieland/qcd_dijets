...
void fillcsv_mc(int j,TH1F* hCSV_mc_c, TH1F* hCSV_mc_b,TH1F* hCSV_mc_lf, float &ptave,float *Jet_CSV, float *Jet_HadFlav, vector<float>Weight, float *Jet_Phi, float *Jet_Eta, int &N_Jets){
	...
	if (N_Jets>=3){
	  float dr=sqrt(pow(TVector2::Phi_mpi_pi(Jet_Phi[0])-TVector2::Phi_mpi_pi(Jet_Phi[2]),2)+pow(Jet_Eta[0]-Jet_Eta[2],2));
	  if (dr>1) fillhisto(hCSV_mc_lf,ptave, Jet_CSV[1], Weight,'m');
	}			  		  
      }
    }
    else if (j==1){		
	...
	if (N_Jets>=3){
	  float dr=sqrt(pow(TVector2::Phi_mpi_pi(Jet_Phi[1])-TVector2::Phi_mpi_pi(Jet_Phi[2]),2)+pow(Jet_Eta[1]-Jet_Eta[2],2));
	  if (dr>1) fillhisto(hCSV_mc_lf,ptave, Jet_CSV[0], Weight,'m');
	}		  
      }
    }  
  }
...
void fillcsv_data(int j,TH1F* hCSV_data, float &ptave,float *Jet_CSV_data, vector<float>Weight,float *Jet_Phi_data, float *Jet_Eta_data, int &N_Jets_data,int hlt40=0,int hlt60=0,int hlt80=0,int hlt140=0,int hlt200=0,int hlt260=0,int hlt320=0,int hlt400=0,int hlt500=0){	        
  if (j==0 ){
    if (N_Jets_data>=3){
      float dr=sqrt(pow(TVector2::Phi_mpi_pi(Jet_Phi_data[0])-TVector2::Phi_mpi_pi(Jet_Phi_data[2]),2)+pow(Jet_Eta_data[0]-Jet_Eta_data[2],2));
      if (dr >1) fillhisto(hCSV_data,ptave, Jet_CSV_data[1], Weight,'d',hlt40,hlt60,hlt80,hlt140,hlt200,hlt260,hlt320,hlt400,hlt500);
    }
  }
  else if (j==1 ){
    if (N_Jets_data>=3){
    float dr=sqrt(pow(TVector2::Phi_mpi_pi(Jet_Phi_data[1])-TVector2::Phi_mpi_pi(Jet_Phi_data[2]),2)+pow(Jet_Eta_data[1]-Jet_Eta_data[2],2));
    if (dr>1) fillhisto(hCSV_data,ptave, Jet_CSV_data[0], Weight,'d',hlt40,hlt60,hlt80,hlt140,hlt200,hlt260,hlt320,hlt400,hlt500);			
    }
  }	    
}
...
//   mcchain->SetBranchAddress("Jet_NSV",Jet_NSV);
  ...
  TH1F* h_dr=new TH1F("h_dr","h_dr",50,0,3);
	...
	if (DeltaPhi > dphicut ){
	  ...
	  for (int j=0;j<2;j++){
	    if (Jet_CSV[j]>0.97){
	      ...
	      if (j==0 && (abs(Jet_HadFlav[1])!=4 && abs(Jet_HadFlav[1])!=5 )){
		if (N_Jets>=3){
		  float dr=sqrt(pow(TVector2::Phi_mpi_pi(Jet_Phi[0])-TVector2::Phi_mpi_pi(Jet_Phi[2]),2)+pow(Jet_Eta[0]-Jet_Eta[2],2));
		  fillhisto(h_dr,ptave, dr, Weight,'m');
		}
	      }
	      else if (j==1 && abs(Jet_HadFlav[0])!=4 && abs(Jet_HadFlav[0])!=5){	
		if (N_Jets>=3){
		  float dr=sqrt(pow(TVector2::Phi_mpi_pi(Jet_Phi[1])-TVector2::Phi_mpi_pi(Jet_Phi[2]),2)+pow(Jet_Eta[1]-Jet_Eta[2],2));
		  fillhisto(h_dr,ptave, dr, Weight,'m');
		}
