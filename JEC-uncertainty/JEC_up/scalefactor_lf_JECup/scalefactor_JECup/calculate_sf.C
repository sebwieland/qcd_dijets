...
//   officialfile->Close();
  TFile* result=new TFile("result_JECup.root","RECREATE");
  //SetColors
//   n_ptbins= 10;
  n_etabins=4;
  for (int i=0; i<n_ptbins;++i){
...
//   }
  ...
  //     //draw histos
      ...
      c1->SaveAs("CSV_ratio_lf_ptbin"+pt_counter+"_etabin"+eta_counter+"_JECup.pdf");
...
//       lf->Write();
      ...
      c2->SaveAs("diff_lf_ptbin"+pt_counter+"_etabin"+eta_counter+"_JECup.pdf");
...
//       lfsf->Write();
      ...
      c2->SaveAs("LFSF_ptbin"+pt_counter+"_etabin"+eta_counter+"_JECup.pdf");
      

      c2->cd();
      //Transform to Histogram
      if ( i!=0 && j!=0){
	TH1F* sf=new TH1F("LFSF_pt"+pt_counter+"_eta"+eta_counter+"_JECup","LFSF_pt"+pt_counter+"_eta"+eta_counter+"_JECup",100000,0,1);
...
int main(){
  TFile* histos=new TFile("../analysetrees_JECup/histos.root");
