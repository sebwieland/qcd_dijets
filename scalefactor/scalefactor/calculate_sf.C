...
//   }
  ...
  float mcevents_lf=0;
  float normratio_lf=0;
  float dataevents_lf=0;
  ...
  //     TH1F* mc=(TH1F*)mc_stack[i][j]->GetStack();  
      mcevents_lf=mc->Integral();
      cout << "bin "<< i <<endl;
      dataevents_lf=h_data[i][j]->Integral();
      cout << "data events: "<< dataevents_lf  << endl;
      ...
      h_b[i][j]->Scale(normratio_lf);
      h_c[i][j]->Scale(normratio_lf);
      h_lf[i][j]->Scale(normratio_lf);
//       h_b[i][j]->Sumw2();
//       h_c[i][j]->Sumw2();
//       h_lf[i][j]->Sumw2();
  ...
  //     //draw histos
      ...
      TH1F* nonb_lf=(TH1F*)h_lf[i][j]->Clone();
      nonb_lf->Add(h_c[i][j]);
      nonb_lf->Draw("SAMEE0");
