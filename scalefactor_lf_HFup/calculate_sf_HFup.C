...
//   officialfile->Close();
  TFile* result=new TFile("result_HFup.root","RECREATE");
  //SetColors
//   n_ptbins= 10;
  n_etabins=4;
  for (int i=0; i<n_ptbins;++i){
...
//   }
  ...
  //     //draw histos
      ...
      c1->SaveAs("CSV_ratio_lf_ptbin"+pt_counter+"_etabin"+eta_counter+"_HFup.pdf");
      ...
      diff_lf->Add(h_b[i][j],-2);
      diff_lf->Add(h_c[i][j],-2);
      diff_lf->Draw("E0");       
...
//       lf->Write();
      ...
      c2->SaveAs("diff_lf_ptbin"+pt_counter+"_etabin"+eta_counter+"_HFup.pdf");
...
//       lfsf->Write();
      ...
      c2->SaveAs("LFSF_ptbin"+pt_counter+"_etabin"+eta_counter+"_HFup.pdf");
      

      c2->cd();
      //Transform to Histogram
      if ( i!=0 && j!=0){
	TH1F* sf=new TH1F("LFSF_pt"+pt_counter+"_eta"+eta_counter+"_HFup","LFSF_pt"+pt_counter+"_eta"+eta_counter+"_HFup",100000,0,1);
