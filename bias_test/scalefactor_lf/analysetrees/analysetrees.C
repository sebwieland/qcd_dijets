...
//     if (ptave < 67.6)				hist->Fill(var,Weight.at(0));
    if (ptave >= 77)   	 	hist->Fill(var,Weight.at(0));
    
  }
  else if (option=='d')
  {
//     if (ptave<67.6 && hlt40==1)                          	hist->Fill(var);
    if (ptave >= 77  && hlt60==1)   		hist->Fill(var);
  }
}
...
void analysetrees(const TString &datalist,const TString &mclist){
  ...
  ///////////////////////
  //Get PtAveWeights
  TFile* f_ptaveweight=new TFile("~/qcd_dijets/bias_test/PtAveWeight/PtAveWeight/ptaveweight.root");
  TH1F* h_ptaveweight = (TH1F*)f_ptaveweight->Get("PtAveWeight");
  TFile* f_PUWeight=new TFile("~/qcd_dijets/bias_test/PU_Reweighting/pu_weight/PUWeight.root");
