...
// tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/DiJets_QCD_Pt_300to470_TuneCUETP8M1_13TeV_pythia8_v3_1.root");
// tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/DiJets_QCD_Pt_300to470_TuneCUETP8M1_13TeV_pythia8_v3_2-pre.root");

tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_1000to1400_TuneCUETP8M1_13TeV_pythia8_v3.root");
// tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_10to15_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_120to170_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_1400to1800_TuneCUETP8M1_13TeV_pythia8_v3.root");
// tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_15to30_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_170to300_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_1800to2400_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_2400to3200_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_300to470_TuneCUETP8M1_13TeV_pythia8_v3.root");
// tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_30to50_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_470to600_TuneCUETP8M1_13TeV_pythia8_v3.root");
// tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_50to80_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_5to10_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_600to800_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_80to120_TuneCUETP8M1_13TeV_pythia8_v3.root");
tree_qcd->AddFile("/nfs/dust/cms/user/matsch/ttHNtuples/Spring15/v3/QCD_Pt_800to1000_TuneCUETP8M1_13TeV_pythia8_v3.root");
...
float Weight_XS;
tree_qcd->SetBranchAddress("Weight_XS",&Weight_XS);
tree_qcd->SetWeight(Weight_XS);
...
TH1F* h_ptavg=new TH1F("h_ptavg","h_ptavg",80,0,220);
TH1F* h_ptavg_data=new TH1F("h_ptavg_data","h_ptavg_data",80,0,220);

TH1F* h_pthardestjet_lf=new TH1F("h_pthardestjet_lf","h_pthardestjet_lf",80,0,220);
TH1F* h_pthardestjet_b=new TH1F("h_pthardestjet_b","h_pthardestjet_b",80,0,220);
TH1F* h_pthardestjet_c=new TH1F("h_pthardestjet_c","h_pthardestjet_c",80,0,220);

TH1F* h_pt_lf=new TH1F("h_pt_lf","h_pt_lf",80,0,220);
TH1F* h_pt_b=new TH1F("h_pt_b","h_pt_b",80,0,220);
TH1F* h_pt_c=new TH1F("h_pt_c","h_pt_c",80,0,220);

TH1F* h_pt2_lf=new TH1F("h_pt2_lf","h_pt_lf",80,0,220);
TH1F* h_pt2_b=new TH1F("h_pt2_b","h_pt_b",80,0,220);
TH1F* h_pt2_c=new TH1F("h_pt2_c","h_pt_c",80,0,220);
...
//   if(iEntry>1000000) break;
  ...
  if(N_Jets >=2){
    ...
    for (int j=0;j< N_Jets;j++){
      h_partonFlav->Fill(Jet_PartFlav[j],Weight_XS);
      if (abs(Jet_PartFlav[j])==4){  
	h_NJets_c->Fill(N_Jets,Weight_XS);
	h_CSV_c->Fill(Jet_CSV[j],Weight_XS);
	}
	    
      else if (abs(Jet_PartFlav[j])==5){ 
	h_NJets_b->Fill(N_Jets,Weight_XS);
	h_CSV_b->Fill(Jet_CSV[j],Weight_XS);
	}
	    
      else if (abs(Jet_PartFlav[j])!=5 && abs(Jet_PartFlav[j])!=4){
	h_NJets_lf->Fill(N_Jets,Weight_XS);
	h_CSV_lf->Fill(Jet_CSV[j],Weight_XS);
	}
      }
	...
	pt_avg=PtAve;
	h_ptavg->Fill(pt_avg,Weight_XS);
	  
	if (abs(Jet_PartFlav[0])==4){
	  h_pthardestjet_c->Fill(Jet_Pt[0],Weight_XS);
	  h_pt2_c->Fill(Jet_Pt[1],Weight_XS);
	}	    
	else if (abs(Jet_PartFlav[0])==5){ 
	  h_pthardestjet_b->Fill(Jet_Pt[0],Weight_XS);
	  h_pt2_b->Fill(Jet_Pt[1],Weight_XS);
	}	  
	else if ( abs(Jet_PartFlav[0])!=5 && abs(Jet_PartFlav[0])!=4){
	  h_pthardestjet_lf->Fill(Jet_Pt[0],Weight_XS);
	  h_pt2_lf->Fill(Jet_Pt[1],Weight_XS);
	}
      
	for (int j=0;j<2;j++){	
	  if (abs(Jet_PartFlav[j])==5 && abs(Jet_HadFlav[j]==5)) {
	    h_SumSVMass_part5_had5->Fill(Jet_SumSVMass[j],Weight_XS);
	    h_NSV_part5_had5->Fill(Jet_NSV[j],Weight_XS);
	  }
	  if (abs(Jet_PartFlav[j])==21 && abs(Jet_HadFlav[j]==5)){
	    h_SumSVMass_part21_had5->Fill(Jet_SumSVMass[j],Weight_XS);
	    h_NSV_part21_had5->Fill(Jet_NSV[j],Weight_XS);
	  }
	  if (abs(Jet_PartFlav[j])==4) h_pt_c->Fill(Jet_Pt[j],Weight_XS);
	  else if (abs(Jet_PartFlav[j])==5) h_pt_b->Fill(Jet_Pt[j],Weight_XS);
	  else if (abs(Jet_PartFlav[j])!=5 && abs(Jet_PartFlav[j])!=4) h_pt_lf->Fill(Jet_Pt[j],Weight_XS);
	  //csv tagging
	  if (Jet_CSV[j]>0.97){
	    if (N_Jets >=3){
		  if (Jet_Pt[2]/pt_avg < pt3cut){
		      if (j==0){
			h_tagjet_partflav_wo_SVMassCut->Fill(abs(Jet_PartFlav[0]),Weight_XS);
			h_flav_probeJet->Fill(abs(Jet_PartFlav[1]),Weight_XS);
		      }
		      else if (j==1){
			h_tagjet_partflav_wo_SVMassCut->Fill(abs(Jet_PartFlav[1]),Weight_XS);
			h_flav_probeJet->Fill(abs(Jet_PartFlav[0]),Weight_XS);
		      }
		}
	    }
		...
		if (N_Jets >=3){
		  if (Jet_Pt[2]/pt_avg < pt3cut){
		      if (j==0){
			h_tagjet_hadflav->Fill(abs(Jet_HadFlav[0]),Weight_XS);
			h_tagjet_partflav->Fill(abs(Jet_PartFlav[0]),Weight_XS);
			if (abs(Jet_PartFlav[1])==4) h_CSV_taggedhigh_c->Fill(Jet_CSV[1],Weight_XS);
			else if (abs(Jet_PartFlav[1])==5) h_CSV_taggedhigh_b->Fill(Jet_CSV[1],Weight_XS);
			else if (abs(Jet_PartFlav[1])!=4 && abs(Jet_PartFlav[1])!=5) h_CSV_taggedhigh_lf->Fill(Jet_CSV[1],Weight_XS);
		      }
		      else if (j==1){
			h_tagjet_hadflav->Fill(abs(Jet_HadFlav[1]),Weight_XS);
			h_tagjet_partflav->Fill(abs(Jet_PartFlav[1]),Weight_XS);
			if (abs(Jet_PartFlav[0])==4) h_CSV_taggedhigh_c->Fill(Jet_CSV[0],Weight_XS);
			else if (abs(Jet_PartFlav[0])==5) h_CSV_taggedhigh_b->Fill(Jet_CSV[0],Weight_XS);
			else if (abs(Jet_PartFlav[0])!=4 && abs(Jet_PartFlav[0])!=5) h_CSV_taggedhigh_lf->Fill(Jet_CSV[0],Weight_XS);
		      }
		}
		}
		else if (j==0 && N_Jets ==2){
		...
		if (N_Jets >=3){
		  if (Jet_Pt[2]/pt_avg < pt3cut){
		      if (j==0){
			if (abs(Jet_HadFlav[1])==4) h_CSV_taggedlow_c->Fill(Jet_CSV[1],Weight_XS);
			else if (abs(Jet_HadFlav[1])==5) h_CSV_taggedlow_b->Fill(Jet_CSV[1],Weight_XS);
			else if (abs(Jet_HadFlav[1])!=4 && abs(Jet_HadFlav[1])!=5) h_CSV_taggedlow_lf->Fill(Jet_CSV[1],Weight_XS);
		      }
		      else if (j==1){
			if (abs(Jet_HadFlav[0])==4) h_CSV_taggedlow_c->Fill(Jet_CSV[0],Weight_XS);
			else if (abs(Jet_HadFlav[0])==5) h_CSV_taggedlow_b->Fill(Jet_CSV[0],Weight_XS);
			else if (abs(Jet_HadFlav[0])!=4 && abs(Jet_HadFlav[0])!=5) h_CSV_taggedlow_lf->Fill(Jet_CSV[0],Weight_XS);
		      }
		  }
		}
		if (j==0 && N_Jets ==2 ){
		  ...
		  else if (abs(Jet_HadFlav[1])!=4 && abs(Jet_PartFlav[1])!=5) h_CSV_taggedlow_lf->Fill(Jet_CSV[1],Weight_XS);
		}
		else if (j==1 && N_Jets ==2){
		  if (abs(Jet_HadFlav[0])==4) h_CSV_taggedlow_c->Fill(Jet_CSV[0],Weight_XS);
		  else if (abs(Jet_HadFlav[0])==5) h_CSV_taggedlow_b->Fill(Jet_CSV[0],Weight_XS);
		  else if (abs(Jet_HadFlav[0])!=4 && abs(Jet_PartFlav[0])!=5) h_CSV_taggedlow_lf->Fill(Jet_CSV[0],Weight_XS);
		}
	  }
	  //true tagging
	  if (truetag==true){
...
TH1F* nonb_lf=(TH1F*)h_CSV_taggedlow_lf->Clone();
