...
float dr(float Jet_Phi0,float Jet_Phi1, float Jet_Eta0,float Jet_Eta1 ){
 return sqrt(pow(TVector2::Phi_mpi_pi(Jet_Phi0)-TVector2::Phi_mpi_pi(Jet_Phi1),2)+pow(Jet_Eta0-Jet_Eta1,2)); 
}

void fillcsv_mc(int j,TH1F* hCSV_mc_c, TH1F* hCSV_mc_b,TH1F* hCSV_mc_lf, float &ptave,float *Jet_CSV, float *Jet_HadFlav, vector<float>Weight, float *Jet_Phi, float *Jet_Eta, int &N_Jets){
	...
	if (N_Jets>=3){
	  float tmp=dr( Jet_Phi[0],Jet_Phi[2], Jet_Eta[0], Jet_Eta[2] ); 
	  if (tmp>1) fillhisto(hCSV_mc_lf,ptave, Jet_CSV[1], Weight,'m');
	}			  		  
      }
    }
    else if (j==1){		
	...
	if (N_Jets>=3){
	  float tmp=dr( Jet_Phi[1],Jet_Phi[2], Jet_Eta[1], Jet_Eta[2] ); 
	  if (tmp>1) fillhisto(hCSV_mc_lf,ptave, Jet_CSV[0], Weight,'m');
	}		  
      }
    }  
  }

void fillcsv_data(int j,TH1F* hCSV_data, float &ptave,float *Jet_CSV_data, vector<float>Weight,float *Jet_Phi_data, float *Jet_Eta_data, int &N_Jets_data,int hlt40=0,int hlt60=0,int hlt80=0,int hlt140=0,int hlt200=0,int hlt260=0,int hlt320=0,int hlt400=0,int hlt500=0){	        
  if (j==0 ){
    if (N_Jets_data>=3){
      float tmp=dr( Jet_Phi_data[0],Jet_Phi_data[2], Jet_Eta_data[0], Jet_Eta_data[2] );      
      if (tmp >1) fillhisto(hCSV_data,ptave, Jet_CSV_data[1], Weight,'d',hlt40,hlt60,hlt80,hlt140,hlt200,hlt260,hlt320,hlt400,hlt500);
    }
  }
  else if (j==1 ){
    if (N_Jets_data>=3){
    float tmp=dr( Jet_Phi_data[1],Jet_Phi_data[2], Jet_Eta_data[1], Jet_Eta_data[2] ) ;
    if (tmp>1) fillhisto(hCSV_data,ptave, Jet_CSV_data[0], Weight,'d',hlt40,hlt60,hlt80,hlt140,hlt200,hlt260,hlt320,hlt400,hlt500);			
    }
  }	    
}
...
//   mcchain->SetBranchAddress("Jet_NSV",Jet_NSV);
  ...
  h_dphi->SetDrawOption("histe0");
  TH1F* h_pt3=new TH1F("h_pt3","h_pt3 w/o PtAveWeight",80,0,220);
  h_pt3->SetDrawOption("histe0");
  TH1F* h_pt3overavg=new TH1F("h_pt3overavg","h_pt3overavg w/o PtAveWeight",60,0,1.5);
  h_pt3overavg->SetDrawOption("histe0");
  TH1F* h_dr_miss=new TH1F("h_dr_miss","h_dr",50,0,4.5);
  TH1F* h_ptrel_miss=new TH1F("h_ptrel_miss","h_ptrel missprobed",80,0,0.5;
  TH1F* h_dr=new TH1F("h_dr","h_dr",50,0,4.5);
  TH1F* h_ptrel=new TH1F("h_ptrel","h_ptrel right probed",80,0,0.5);
...
//     if(iEntry>1000000) break;
	...
	if (DeltaPhi > dphicut ){
	  ...
	  for (int j=0;j<2;j++){
	    if (abs(Jet_HadFlav[j]==4)){
	      fillcsv_mc(j,hCSV_mc_c[0][0], hCSV_mc_b[0][0], hCSV_mc_lf[0][0],ptave, Jet_CSV,Jet_HadFlav ,Weight, Jet_Phi, Jet_Eta, N_Jets);
	      
	      if (j==0 && (abs(Jet_HadFlav[1])!=5  )){		
		for (int i=0;i<N_Jets;++i){
		  if (i!=0){
		    float tmp=dr(Jet_Phi[0],Jet_Phi[i], Jet_Eta[0], Jet_Eta[i] ) ;
		    fillhisto(h_dr_miss,ptave, tmp, Weight,'m');
		    if ( tmp<1.5) {		   
		      fillhisto(h_ptrel_miss,ptave, Jet_Pt[i]/Jet_Pt[0], Weight,'m');
		    }		  
		  }
		}
	      }
	      else if (j==1 && abs(Jet_HadFlav[0])!=5){	
		for (int i=0;i<N_Jets;++i){
		  if (i!=1){
		    float tmp=dr(Jet_Phi[1],Jet_Phi[i], Jet_Eta[1], Jet_Eta[i] ) ;
		    fillhisto(h_dr_miss,ptave, tmp, Weight,'m');
		    if ( tmp<1.5) {
		      fillhisto(h_ptrel_miss,ptave, Jet_Pt[i]/Jet_Pt[1], Weight,'m');
		    }		  
		  }
		} 
	      }
		...
		for (int i=0;i<N_Jets;++i){
		  if (i!=0){
		    float tmp=dr(Jet_Phi[0],Jet_Phi[i], Jet_Eta[0], Jet_Eta[i] ) ;
		    fillhisto(h_dr,ptave, tmp, Weight,'m');
		  if ( tmp<1.5) {		   
		    fillhisto(h_ptrel,ptave, Jet_Pt[i]/Jet_Pt[0], Weight,'m');
		  }		  
		 }
		}
	      }
		...
		for (int i=0;i<N_Jets;++i){
		  if (i!=1){
		    ...
		    fillhisto(h_dr,ptave, tmp, Weight,'m');
		    if ( tmp<1.5) {
		      fillhisto(h_ptrel,ptave, Jet_Pt[i]/Jet_Pt[1], Weight,'m');
		    }			  
		  }
		}
	      }
...
//   datachain->SetBranchAddress("Jet_NSV",Jet_NSV_data);
  ...
  for (long iEntry=0;iEntry<nentries;iEntry++) {
	...
	if (DeltaPhi_data> dphicut ){
	  ...
	  for (int j=0;j<2;j++){
	    if (Jet_CSV_data[j]>0.97 ){      
	      fillcsv_data(j,hCSV_data[0][0], ptave, Jet_CSV_data, Weight, Jet_Phi_data, Jet_Eta_data, N_Jets_data,hlt40, hlt60, hlt80, hlt140, hlt200, hlt260, hlt320, hlt400, hlt500);	      
	      if(j==0){
